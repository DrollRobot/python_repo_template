"""Unit tests for scripts/template_setup/remove_security_policy.py.
...
scripts/template_setup/remove_security_policy.py and deletes it along with the
...
def test_plan_deletions_is_empty_when_the_file_is_gone(tmp_path: Path) -> None:
    """A project that already removed SECURITY.md plans nothing."""
    assert remove_security_policy.plan_deletions(tmp_path) == []


@pytest.mark.unit
def test_plan_deletions_finds_the_security_policy(tmp_path: Path) -> None:
    """The policy file is planned for deletion when it exists."""
    _make_project(tmp_path, "SECURITY.md")

    deletions = remove_security_policy.plan_deletions(tmp_path)
    assert [path.name for path in deletions] == ["SECURITY.md"]
...
def test_run_returns_zero_when_nothing_matches(tmp_path: Path) -> None:
    ...
    assert remove_security_policy.run(tmp_path, assume_yes=True) == 0
...
def test_run_dry_run_deletes_nothing(tmp_path: Path) -> None:
    ...
    (path,) = _make_project(tmp_path, "SECURITY.md")

    assert remove_security_policy.run(tmp_path, assume_yes=True, dry_run=True) == 0
    assert path.exists()


@pytest.mark.integration
@pytest.mark.functional
def test_run_deletes_the_security_policy(tmp_path: Path) -> None:
    """A real run deletes SECURITY.md."""
    (path,) = _make_project(tmp_path, "SECURITY.md")
...
def test_run_leaves_unrelated_files_alone(tmp_path: Path) -> None:
    ...
    _make_project(tmp_path, "SECURITY.md")
    (kept,) = _make_project(tmp_path, "CONTRIBUTING.md")
