"""Unit tests for the no-chained-commands-bash PreToolUse hook.

The hook lives at .claude/hooks/no-chained-commands-bash.py. Its filename is not
...
Unlike the pwsh variant, this one gates on tool_name == "Bash".
"""
...
_HOOK_PATH = (
    Path(__file__).resolve().parent.parent / ".claude" / "hooks" / "no-chained-commands-bash.py"
)
_spec = importlib.util.spec_from_file_location("no_chained_commands_bash", _HOOK_PATH)
...
def test_non_bash_tool_allows(
    ...
    # The gate: a chained PowerShell command is left to the pwsh hook.
    rc, err = _invoke(
        monkeypatch, capsys, {"tool_name": "PowerShell", "tool_input": {"command": "a; b"}}
    )
    assert rc == 0
    assert err == ""
...
def test_and_and_blocks(
    ...
    rc, err = _invoke(monkeypatch, capsys, _bash("git add . && git commit -m x"))
    assert rc == 2
...
def test_multiple_joiners_all_reported(
    ...
    rc, err = _invoke(monkeypatch, capsys, _bash("a && b; c"))
    assert rc == 2
    assert "joined with '&&'" in err
    assert "joined with ';'" in err
...
def test_single_command_allowed(
    ...
    rc, err = _invoke(monkeypatch, capsys, _bash("git status"))
    assert rc == 0
    assert err == ""
...
def test_semicolon_inside_single_quotes_allowed(
    ...
    rc, err = _invoke(monkeypatch, capsys, _bash("git commit -m 'fix; the bug'"))
    assert rc == 0
    assert err == ""
...
def test_joiner_inside_double_quotes_allowed(
    ...
    rc, err = _invoke(monkeypatch, capsys, _bash('echo "a && b"'))
    assert rc == 0
    assert err == ""
